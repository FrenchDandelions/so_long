...
bool	again(char **map, int i, int j, t_tretris *valid)
{
	if (map[i][j] == EXIT
		...
		return (true);
	if (i + 1 < valid->height && map[i + 1][j] != WALL && map[i + 1][j] != 'B'
		&& map[i + 1][j] != 'U')
		validpath(map, i + 1, j, valid);
	if (i - 1 > 0 && map[i - 1][j] != WALL && map[i - 1][j] != 'B' && map[i
		- 1][j] != 'U')
		validpath(map, i - 1, j, valid);
	if (j - 1 >= 0 && map[i][j - 1] != WALL && map[i][j - 1] != 'B' && map[i][j
		- 1] != 'U')
		validpath(map, i, j - 1, valid);
	if (j + 1 < valid->width && map[i][j + 1] != WALL && map[i][j + 1] != 'B'
		&& map[i][j + 1] != 'U')
		validpath(map, i, j + 1, valid);
	return (false);
}
